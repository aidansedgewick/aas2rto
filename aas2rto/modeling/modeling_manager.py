...
def modeling_wrapper(func: Callable, target: Target, t_ref: Time = None):
    try:
...
class ModelingManager:
    ...
    def __init__(
        ...
        if not isinstance(nworkers, int):
            msg = f"modeling_manager 'nworkers' must be integer, not {type(nworkers)}"
    ...
    def build_target_models(
        ...
        for modeling_func in modeling_functions:
            ...
            if self.config["nworkers"] == 1:
                ...
                logger.info(f"build with {nworkers} Pool workers")
                with Pool(nworkers, maxtasksperchild=10) as pool:
