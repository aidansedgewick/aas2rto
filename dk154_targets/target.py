...
class TargetData:
    ...
    def add_lightcurve(
        ...
        alltags = detection_tags + badqual_tags + nondet_tags
        if tag_col in self.lightcurve.columns:
            ...
            unknown_tag_mask = np.isin(lightcurve[tag_col], alltags)
            if not all(unknown_tag_mask):
                unknown_tags = self.lightcurve[tag_col][unknown_tag_mask]
...
class Target:
    ...
    def build_model(self, modeling_function: Callable, t_ref: Time = None):
        ...
        try:
            ...
            key_name = type(modeling_function).__name__
            msg = (
                f"\n    Your modeling_function {key_name} should have attribute __name__."
            )
    ...
    def plot_observing_chart(
        self, observatory, t_ref: Time = None, sun=None, moon=None, fig_path=None
    ):
        obs_name = getattr(observatory, "name", "no_observatory")
        t_ref = t_ref or Time.now()
        ...
        try:
            oc_fig = plot_observing_chart(
                observatory, self, sun=sun, moon=moon, t_ref=t_ref
            )
        except Exception as e:
    ...
    def write_comments(self, outdir: Path, t_ref: Time = None):
        ...
        print(f"WRITE TO {comments_file}")
...
class DefaultLightcurvePlotter:
    ...
    plot_colors = {**ztf_colors, **atlas_colors}
    ...
    def plot(cls, target: Target, t_ref: Time = None) -> plt.Figure:
        ...
        plotter.format_axes(target, t_ref=t_ref)
    ...
    def plot_photometry(self, target: Target, t_ref: Time = None):
        t_ref = t_ref or Time.now()
    ...
    def format_axes(self, target, t_ref):
        t_ref = t_ref or Time.now()
        ...
        self.ax.axvline(t_ref.jd - t_ref.jd, color="k")
    ...
    def add_readable_dateticks(self, t_ref):
        twiny = self.ax.twiny()
        ...
        xticks = t_ref.jd + np.arange(xmin, xmax, s)
        xticklabels = [Time(int(x), format="jd").strftime("%d %b") for x in xticks]
        twiny.set_xticks(xticks - t_ref.jd)
...
def plot_observing_chart(
    ...
    plotter = ObservingChartPlotter.plot(observatory, *target_list)
    if return_plotter:
        return plotter
    return plotter.fig
...
class ObservingChartPlotter:
    ...
    def __init__(
        ...
        if t_grid is None:
            t_grid = t_ref + np.arange(0, self.forecast + self.dt, self.dt) * u.day
        self.t_grid = t_grid
        self.legend_handles = []
        if obs_info is None:
            obs_info = ObservatoryInfo.for_observatory(
                self.observatory, t_grid=self.t_grid
            )
