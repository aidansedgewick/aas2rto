...
class Test__BuildModel:
    def test__build_simple_model(self, test_target):
        ...
        assert set(test_target.models_t_ref.keys()) == set(["simple_model"])
        assert isinstance(test_target.models_t_ref["simple_model"], Time)
        assert np.isclose(test_target.models_t_ref["simple_model"].mjd, 60000.0)

    def test__model_updated(self, test_target):
        t_ref = Time(60000.0, format="mjd")
        t_future = Time(60050.0, format="mjd")

        model_func1 = BuildSimpleModel()
        model_func2 = BuildSimpleModel(chi2=3.0)

        test_target.build_model(model_func1, t_ref=t_ref)
        model = test_target.models.get("simple_model_class", None)
        model_t_ref = test_target.models_t_ref.get("simple_model_class", None)
        assert isinstance(model, SimpleModel)
        assert np.isclose(model.chi2, 1.0)
        assert np.isclose(model_t_ref.mjd, 60000.0)

        test_target.build_model(model_func2, t_ref=t_future)
        model = test_target.models.get("simple_model_class", None)
        model_t_ref = test_target.models_t_ref.get("simple_model_class", None)
        assert np.isclose(model.chi2, 3.0)
        assert np.isclose(model_t_ref.mjd, 60050.0)
    ...
    def test__tref_updated(self, test_target):
        ...
        assert np.isclose(test_target.models_t_ref["simple_model"].mjd, 60050.0)
...
class Test__DefaultLightcurvePlotter:
    ...
    def tset__no_exception_plot_photometry_blank(self, test_target, tmp_path):
        ...
        plotter.plot_photometry(test_target)
...
class Test__ObservingChartPlotter:
    ...
    def test__plot_method_no_alt(self, test_observer, test_target, tmp_path):
        ...
        ObservingChartPlotter.dt = 1.0 / 24.0
