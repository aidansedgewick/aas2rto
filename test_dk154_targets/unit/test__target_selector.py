...
from astroplan import Observer, TargetNeverUpWarning, TargetAlwaysUpWarning
...
@pytest.fixture
def empty_selector():
    return TargetSelector({}, create_paths=False)


@pytest.fixture
def test_target_list():
    return [
        Target("t1", ra=30.0, dec=10.0),
        Target("t2", ra=60.0, dec=10.0),
        Target("t3", ra=90.0, dec=10.0),
    ]


@pytest.fixture
def test_target_data_list():
    dict_list = [
        dict(
    ...
    return [pd.DataFrame(d) for d in dict_list]


@pytest.fixture
def test_target_list_with_data(test_target_list, test_target_data_list):
    test_target_list[0].alerce_data.add_lightcurve(test_target_data_list[0])
    test_target_list[1].alerce_data.add_lightcurve(test_target_data_list[1])
    test_target_list[2].alerce_data.add_lightcurve(test_target_data_list[2])
    return test_target_list


class TestTargetSelector:
    def test__selector_init(self):
        empty_config = {}
    ...
    def test__from_config(self):
        ...
        assert np.isclose(
            selector.observatories["lasilla"].location.lon, -70.73 * u.deg
        )
        assert np.isclose(
            selector.observatories["astrolab"].location.lat, 54.75 * u.deg
        )
    ...
    def test__add_target(self, empty_selector):
        t1 = Target("t1", ra=10.0, dec=30.0)

        selector = TargetSelector({})

    def test__perform_all_qm_tasks(self):
        class TestObject:
        ...
        selector = TargetSelector({})
        selector.query_managers["test_selector"] = TestQueryManager(
            selector.target_lookup
        )
    ...
    def test__compute_observatory_info(self, test_target_list):
        ...
        assert np.isclose(
            astrolab_sunset.mjd - 60000.0,
            exp_astrolab_sunset.mjd - 60000,
            atol=time_atol,
        )
        ...
        assert np.isclose(
            astrolab_sunset.mjd - 60000.0,
            exp_astrolab_sunset.mjd - 60000.0,
            atol=time_atol,
        )
        ...
        assert np.isclose(
            lasilla_sunset.mjd - 60000.0,
            exp_lasilla_sunset.mjd - 60000.0,
            atol=time_atol,
        )
        assert np.isclose(
            lasilla_sunrise.mjd - 60000.0,
            exp_lasilla_sunrise.mjd - 60000.0,
            atol=time_atol,
        )

    def test__observatory_info_no_crash_on_tonight_failure(self):
        config = {
            "observatories": {
        ...
        t_ref = Time("2022-12-21 12:00:00", format="iso")  # Noon UT, solstice
        ...
        with warnings.catch_warnings():
            ...
            warnings.filterwarnings("ignore", category=TargetAlwaysUpWarning)  # SPT
            warnings.filterwarnings("ignore", category=TargetNeverUpWarning)  # NPT
            selector.compute_observatory_info(t_ref=t_ref)
    ...
    def test__initial_target_check(self, test_target_list):
        config = {
    ...
    def test__evaluate_all_targets(self, test_target_list):
        config = {
            "observatories": {"palomar": "palomar"},
        }
    ...
    def test__remove_bad_targets(self, test_target_list):
        config = {
            "observatories": {"palomar": "palomar"},
        }
    ...
    def test__compile_target_lightcurves(
        self, empty_selector, test_target_list_with_data
    ):
        for target in test_target_list_with_data:
            empty_selector.add_target(target)
        assert all([t.compiled_lightcurve is None for t in test_target_list_with_data])

        t_ref = Time(60003.8, format="mjd")  # cut off the last datapoint
        empty_selector.compile_target_lightcurves(t_ref=t_ref)

        assert all(
            ["jd" in t.compiled_lightcurve.columns for t in test_target_list_with_data]
        )
        assert all(
            [len(t.compiled_lightcurve) == 4 for t in test_target_list_with_data]
        )
        expected_t1_values = [19.0, 18.0, 17.0, 16.0]
        assert np.allclose(
            empty_selector.target_lookup["t1"].compiled_lightcurve["mag"],
            expected_t1_values,
        )
        expected_t2_values = [19.1, 18.1, 17.1, 16.1]
        assert np.allclose(
            empty_selector.target_lookup["t2"].compiled_lightcurve["mag"],
            expected_t2_values,
        )
        expected_t3_values = [19.2, 18.2, 17.2, 16.2]
        assert np.allclose(
            empty_selector.target_lookup["t3"].compiled_lightcurve["mag"],
            expected_t3_values,
        )

    def test__opp_target_loading(self):
    ...
    def test__build_target_models(
        self, empty_selector, test_target_list, test_target_data_list
    ):
        class BasicExampleModel:
        ...
        def example_model(target):
            return BasicExampleModel(x=len(target.alerce_data.lightcurve))

        def another_example_model(target):
            return AnotherExampleModel(x=len(target.alerce_data.lightcurve))

        function_list = [example_model, another_example_model]

        for target, data in zip(test_target_list, test_target_data_list):
            target.alerce_data.add_lightcurve(data.iloc[:-1])
            assert len(target.alerce_data.lightcurve) == 4

        for target in test_target_list:
            empty_selector.add_target(target)
        assert set(empty_selector.target_lookup.keys()) == set(["t1", "t2", "t3"])

        test_target_list[0].updated = True
        test_target_list[1].updated = True
        test_target_list[2].updated = False

        empty_selector.build_target_models(example_model)

        # Target1 will have a model
        assert set(test_target_list[0].models.keys()) == set(["example_model"])
        assert isinstance(
            test_target_list[0].models["example_model"], BasicExampleModel
        )
        assert test_target_list[0].models["example_model"].x == 4
        # Target2 will have a model
        assert set(test_target_list[1].models.keys()) == set(["example_model"])
        assert isinstance(
            test_target_list[1].models["example_model"], BasicExampleModel
        )
        assert test_target_list[1].models["example_model"].x == 4
        # Target3 will NOT have a model
        assert set(test_target_list[2].models.keys()) == set()

        for target, data in zip(test_target_list, test_target_data_list):
            target.alerce_data.add_lightcurve(data)
            assert len(target.alerce_data.lightcurve) == 5

        test_target_list[0].updated = True
        test_target_list[1].updated = False
        test_target_list[2].updated = False

        empty_selector.build_target_models([example_model, another_example_model])

        # Target1 will have been updated.
        assert set(test_target_list[0].models.keys()) == set(
            ["example_model", "another_example_model"]
        )
        assert isinstance(
            test_target_list[0].models["example_model"], BasicExampleModel
        )
        assert isinstance(
            test_target_list[0].models["another_example_model"], AnotherExampleModel
        )
        assert test_target_list[0].models["example_model"].x == 5
        assert test_target_list[0].models["another_example_model"].x == 5

        # Target2 will be the same as before!
        assert set(test_target_list[1].models.keys()) == set(["example_model"])
        assert isinstance(
            test_target_list[1].models["example_model"], BasicExampleModel
        )
    ...
    def test__reset_updated_target_flags(self, test_target_list):
        selector = TargetSelector({})
        for target in test_target_list:
            selector.add_target(target)
