...
class Test__FinkQueryManagerInit:
    ...
    def test__bad_fink_kafka_config(self, kafka_config, target_lookup, tmp_path):
        ...
        assert all(
            [
                kw in bad_kafka_config
                for kw in ["group_id", "bootstrap.servers", "topics"]
            ]
        )
        ...
        assert all(
            [
                kw in bad_kafka_config
                for kw in ["username", "bootstrap.servers", "topics"]
            ]
        )
        config = {"kafka_config": bad_kafka_config}
        with pytest.raises(BadKafkaConfigError):
        ...
        assert all(
            [kw in bad_kafka_config for kw in ["username", "group_id", "topics"]]
        )
        config = {"kafka_config": bad_kafka_config}
        with pytest.raises(BadKafkaConfigError):
            qm = FinkQueryManager(
        ...
        assert all(
            [
                kw in bad_kafka_config
                for kw in ["username", "group_id", "bootstrap.servers"]
            ]
        )
        config = {"kafka_config": bad_kafka_config}
        with pytest.raises(BadKafkaConfigError):
