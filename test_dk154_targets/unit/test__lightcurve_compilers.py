...
def test__prepare_atlas_data():
    ...
    formatted_df = prepare_atlas_data(fake_lc)

    expected_columns = "mag magerr diffmaglim band jd tag".split()
...
def test__default_compile_lightcurve():
    alerce_dict = dict(
    ...
    alerce_lc = pd.DataFrame(alerce_dict)
    alerce_data = TargetData(lightcurve=alerce_lc)
    ...
    atlas_lc = pd.DataFrame(atlas_dict)
    atlas_data = TargetData(lightcurve=atlas_lc)
